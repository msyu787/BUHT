...
SDXL + ControlNet + LoRA로 생성한 한국 수묵화를 4가지 지표로 평가하고,
순정(baseline) SDXL 생성 결과와 비교하는 표를 출력한다.
...
수정 이력
---------
[2026-07-24] CLIP 임베딩 추출 호환성 수정
  무엇을: `_as_tensor()` 헬퍼를 새로 추가하고, CLIP 임베딩을 꺼내는 세 곳
          (`_clip_image_embeddings`, `q_sem`의 이미지/텍스트 임베딩)에서
          이 헬퍼를 거치도록 변경.
  왜:     transformers 버전에 따라 `get_image_features` / `get_text_features` 가
          임베딩 텐서가 아니라 `BaseModelOutputWithPooling` 객체를 반환하는 경우가 있다.
          그 상태로 `.norm()` 을 호출하면
          `AttributeError: 'BaseModelOutputWithPooling' object has no attribute 'norm'`
          이 발생해 q_sem / q_copy 가 실행되지 않았다.
  방법:   반환값이 이미 텐서면 그대로 쓰고, 아니면
          image_embeds → text_embeds → pooler_output 순으로 텐서를 찾아 꺼낸다.
          (`visual_projection` 을 다시 태우면 차원이 맞지 않아 shape 오류가 나므로
           pooler_output 을 그대로 최종 임베딩으로 사용한다.)
  참고:   같은 방식으로 ours / baseline 양쪽을 계산하므로 상대 비교는 유효하다.

[2026-07-24] 베이스 모델 표기를 SD v1.5 → SDXL 로 정정
  무엇을: 문서 문자열, BASELINE_DIR 주석, 비교표 라벨(baseline_sd15 → baseline_sdxl).
  왜:     초기에는 Colab T4 의 OOM 때문에 SD v1.5 로 실험했으나,
...
BASELINE_DIR = OUTPUT_ROOT / "generated" / "baseline"  # 순정 SDXL 생성 결과
...
def _as_tensor(feat):
    """
    CLIP의 get_image_features / get_text_features 반환값을 텐서로 통일한다.

    transformers 버전에 따라 임베딩 텐서가 아니라
    BaseModelOutputWithPooling 객체가 반환되는 경우가 있어,
    그럴 때는 pooler_output(이미 최종 임베딩 차원)을 꺼내 쓴다.
    """
...
def _clip_image_embeddings(paths: list[Path]) -> torch.Tensor:
    ...
    for i in range(0, len(paths), BATCH_SIZE):
        ...
        # [수정 2026-07-24] transformers 버전에 따라 텐서가 아닌 객체가 반환될 수 있어
        # _as_tensor() 로 감싼다. (원래 코드: emb = model.get_image_features(**inputs))
...
def q_sem(gen_dir: Path = OURS_DIR) -> dict:
    ...
    for i in range(0, len(pairs), BATCH_SIZE):
        ...
        # [수정 2026-07-24] 아래 두 임베딩도 같은 이유로 _as_tensor() 를 거친다.
        img_emb = _as_tensor(
...
def main():
    ...
    targets = [("ours", OURS_DIR), ("baseline_sdxl", BASELINE_DIR)]
    rows = []
